...
const FIELD_LABELS: Record<string, { label: string; icon: string }> = {
  name:          { label: 'שם מלא',       icon: '👤' },
  phone:         { label: 'טלפון',        icon: '📞' },
  email:         { label: 'אימייל',        icon: '✉️' },
  business_type: { label: 'תחום עיסוק',   icon: '🏢' },
  notes:         { label: 'פרטים נוספים', icon: '✏️' },
}
...
export function LandingPageTemplate({ page, heroImageUrl, logoUrl, previewMode = false }: Props) {
  ...
  const social = [
    { key: 'phone',     icon: PhoneIcon,     label: 'חייגו עכשיו',            value: s.phone },
    { key: 'whatsapp',  icon: WhatsAppIcon,  label: 'שלחו הודעה בוואטסאפ',   value: s.whatsapp },
    { key: 'instagram', icon: InstagramIcon, label: 'עקבו אחרינו באינסטגרם', value: s.instagram_url },
    { key: 'facebook',  icon: FacebookIcon,  label: 'בקרו אותנו בפייסבוק',   value: s.facebook_url },
    { key: 'email',     icon: EmailIcon,     label: 'שלחו מייל',              value: s.email_contact },
  ]

  const formAction = previewMode ? '#' : `/api/submit/${page.token}`
  ...
  return (
    ...
    >
      {/* ── Row: 75% image | 25% form ── */}
      <div style={{ display: 'flex', flex: 1, minHeight: 520 }}>

        {/* col-9 image — right side in RTL */}
        <div style={{ flex: '0 0 75%', overflow: 'hidden', minHeight: 0, position: 'relative' }}>
          {/* Background image */}
          {heroImageUrl ? (
            <img
              src={heroImageUrl}
        ...
        {/* col-3 form (25%) — left side in RTL */}
        <div
          style={{
            flex: '0 0 25%',
            display: 'flex',
        ...
        >
          ...
          {logoUrl && (
            <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 20 }}>
              <img src={logoUrl} alt="logo" style={{ width: '70%', height: 64, objectFit: 'contain', display: 'block' }} />
            </div>
          )}
          ...
          >
            {FIXED_FIELDS.map((field) => {
              const { label } = FIELD_LABELS[field] ?? { label: field }
              return field === 'notes' ? (
                <textarea
                  key={field}
                  name={field}
      ...
      >
        {social.map(({ key, icon: Icon, label, value }) => (
          <a
            key={key}
            href={value || '#'}
            target={value && value !== '#' ? '_blank' : undefined}
            rel="noreferrer"
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, textDecoration: 'none', color: 'inherit' }}
            onClick={previewMode ? (e) => e.preventDefault() : undefined}
          >
            <div style={{ width: 38, height: 38, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1.5px solid ${accent}`, color: accent }}>
              <Icon size={16} />
            </div>
            <span style={{ fontSize: 11, color: '#9ca3af', whiteSpace: 'nowrap' }}>{label}</span>
            {(key === 'phone' || key === 'whatsapp') && value && (
              <span style={{ fontSize: 11, fontFamily: 'monospace', color: accent, opacity: 0.8 }}>{value}</span>
            )}
          </a>
        ))}
      </div>
